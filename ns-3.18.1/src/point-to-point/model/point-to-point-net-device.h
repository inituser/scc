...
public:
  ...
  //scc, xl
  Ptr<Queue> m_input_queue;
  Ptr<Queue> GetInputQueue (void) const;
  void SetInputQueue (Ptr<Queue> queue);
  uint8_t m_scc_mode;
  uint32_t m_scc_k;

  TracedCallback< Ptr<const Packet>, uint32_t*,  std::string& > m_scc_isack;
  TracedCallback< Ptr<const Packet>, uint32_t*  >               m_scc_isfin;
  ...
  enum SCCState
  {
    UCS,   //uncongested state
    CS,    //congested state
	LCS,   //light congested state
	FS     //full state
  };
  SCCState m_scc_state;
